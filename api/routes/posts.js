...
router.post('/get_list_posts', async (req, res) => {
    var {token, index, count, lastID} = req.query;
    ...
    // PARAMETER_TYPE_IS_INVALID
    if((index && typeof index !== "string") || (count && typeof count !== "string") || (lastID && typeof lastID !== "string")
        || (token && typeof token !== "string")) {
        console.log("PARAMETER_TYPE_IS_INVALID");
        return setAndSendResponse(res, responseError.PARAMETER_TYPE_IS_INVALID);
    }
    ...
    try {
        ...
        posts = await Post.find().populate('author likedUser').sort("-created");
    } catch (err) {
        return setAndSendResponse(res, responseError.CAN_NOT_CONNECT_TO_DB);
    }
    ...
    let index_last_id = Math.max(posts.findIndex((element) => {return element._id == lastID}), 0);
    console.log('index_last_id: ', index_last_id)
    
    let slicePosts
    if (index_last_id != 0) {
        const lastIdToGet = Math.min(index_last_id + index + count + 1, posts.length);
        slicePosts = posts.slice(index_last_id + index + 1, lastIdToGet);
    } else {
        const lastIdToGet = Math.min(index_last_id + index + count, posts.length);
        slicePosts = posts.slice(index_last_id + index, lastIdToGet);
    }
    const newLastID = slicePosts.length > 0 ? slicePosts[slicePosts.length - 1]._id : 0;
...
router.post('/add_post', cpUpload, verify, async (req, res, next) => {
    ...
    if(!described || !status) {
        console.log("Khong co described");
        return setAndSendResponse(res, responseError.PARAMETER_IS_NOT_ENOUGH);
    }
